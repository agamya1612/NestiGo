...
NestiGo is an enterprise-grade, highly scalable, and event-driven microservices architecture built for a multi-vertical delivery and home-services platform.

The system leverages the Saga Pattern for distributed transactions, ensuring strict data consistency and fault tolerance. For example, if a provider is unavailable or a payment fails, the system automatically triggers asynchronous rollback events across all relevant microservices (e.g., refunding the wallet, cancelling the order, restoring inventory limits).
...
- Performance: Load tested to handle 1,000+ concurrent virtual users with 0.00% 5xx error rate.
...
7. catalog-service: Manages the inventory of services available for purchase and atomic stock deduction constraints.
...
10. payment-service: Processes webhook events and manages automated commission settlements with idempotency checks.
...
14. admin-service: Internal dashboard API for verifying restricted pharma orders, managing users, and monitoring global health.
15. audit-service: An immutable "black-box" Kafka sink that records every single event across the cluster for strict compliance.
