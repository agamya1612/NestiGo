...
const testIdempotency = async () => {
  ...
  logTest('Webhook Second Try (Idempotency)', res2.data.status, 'ignored');
};
...
const testInventoryConcurrency = async () => {
  console.log('\n--- 5. Inventory Concurrency Stress Test ---');
  
  const numRequests = 10;
  console.log(`Firing ${numRequests} concurrent inventory deductions...`);

  const promises = [];
  for (let i = 0; i < numRequests; i++) {
    promises.push(axios.post(`${API_GATEWAY}/api/inventory/deduct`, {
      catalogItemId: '77777777-7777-7777-7777-777777777772', // Amoxicillin (mock has 50)
  ...
  try {
    ...
    logTest(`Concurrent Deductions Processed`, successful, numRequests);
  } catch (err) {
    console.error('❌ Inventory Test Failed:', err.message);
    process.exit(1);
  }
};
